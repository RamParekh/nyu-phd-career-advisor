...
# Copy the functions from app1.py
...
# Test the app's sentiment calculation
print("Testing app's sentiment calculation...")
...
print(f"\nFinal sentiment scores: {sentiment_scores}")

# Test division mapping
test_division = "Economics"
mapped_group = map_division_to_major_group(test_division)
print(f"\nDivision '{test_division}' maps to '{mapped_group}'")

if sentiment_scores and mapped_group in sentiment_scores:
    data = sentiment_scores[mapped_group]
    print(f"✅ Found sentiment data for {mapped_group}:")
    print(f"  Average sentiment: {data['average_sentiment']:.3f}")
    print(f"  Comment count: {data['comment_count']}")
    print(f"  Sentiment range: {data['sentiment_range'][0]:.3f} to {data['sentiment_range'][1]:.3f}")
else:
    print(f"❌ No sentiment data found for {mapped_group}")
    print(f"Available groups: {list(sentiment_scores.keys()) if sentiment_scores else 'None'}") 